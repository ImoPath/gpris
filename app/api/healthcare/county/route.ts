...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.log("[healthcare/county] query row counts:", {
      county: name,
      regResult: regResult.rowCount ?? 0,
      facResult: facResult.rowCount ?? 0,
      facLevels: facLevels.rowCount ?? 0,
    });
    console.log("[healthcare/county] aggregate snapshot:", {
      county: name,
      registrations: r ?? null,
      facilities: f ?? null,
      firstFacilityLevel: facLevels.rows[0] ?? null,
    });
