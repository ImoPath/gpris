...
export async function GET() {
  try {
    ...
    console.log("[healthcare/summary] query row counts:", {
      regStats: regStats.rowCount ?? 0,
      regByCounty: regByCounty.rowCount ?? 0,
      facStats: facStats.rowCount ?? 0,
      facByLevel: facByLevel.rowCount ?? 0,
      facByCounty: facByCounty.rowCount ?? 0,
      syncState: syncState.rowCount ?? 0,
    });
    console.log("[healthcare/summary] aggregate snapshot:", {
      registrations: reg,
      facilities: fac,
      sampleRegistrationCounty: regByCounty.rows[0] ?? null,
      sampleFacilityCounty: facByCounty.rows[0] ?? null,
      sampleFacilityLevel: facByLevel.rows[0] ?? null,
      sampleSyncRow: syncState.rows[0] ?? null,
    });
