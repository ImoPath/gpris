...
const pillarCards: PillarCard[] = [
  ...
  },
  {
    ...
    title: "MSME Economy",
    description:
      "Empowering small businesses through financing and market access.",
  ...
  },
  {
    ...
    description:
      "Affordable homes delivered for low and middle-income households.",
    detailHref: "/housing",
    icon: "Home",
    color: "#3B82F6",
  },
  {
    ...
    description:
      "Expanding access to quality, affordable healthcare nationwide.",
  ...
  },
  {
    ...
    detailHref: "/digital-superhighway-and-creative-economy",
    icon: "Wifi",
    color: "#3B82F6",
  },
];
...
const heroSliderImages = [
  {
    src: "https://res.cloudinary.com/dirib3jmw/image/upload/q_auto/f_auto/v1775550856/47349e5c56fe81cbb46dd533f2586a9f6cc564e5_w33okx.jpg",
    alt: "Kenya Railway",
  },
  {
    src: "https://res.cloudinary.com/dirib3jmw/image/upload/q_auto/f_auto/v1775550856/9c0e5f10fb6a110359abb27da8490d9fe9647965_rjqbz3.jpg",
    alt: "Kenya Stadium",
  },
  {
    src: "https://res.cloudinary.com/dirib3jmw/image/upload/q_auto/f_auto/v1773815336/skyline_nkip6b.jpg",
    alt: "Kenya Bridge with lighting",
  },
  {
    src: "https://res.cloudinary.com/dirib3jmw/image/upload/q_auto/f_auto/v1775550856/0e31e373134c55464879e91417d3dd5e93af7043_m5iwux.jpg",
    alt: "Kenya Building modern architecture",
  },
  {
    src: "https://res.cloudinary.com/dirib3jmw/image/upload/q_auto/f_auto/v1775550856/7a0583c5df76d852cf2bf1d3bd8d121ab4555f5f_vrcnwg.jpg",
    alt: "Kenya Power Plant",
  },
];
...
function extractTopServices(huduma: HudumaResponse | null): ServiceStat[] {
  ...
  const raw = (huduma.services ??
    huduma.topServices ??
    huduma.top_mda_services ??
    huduma.mda_services) as unknown;
  if (!raw) return [];
  ...
  if (Array.isArray(raw)) {
    return raw
      .map((item) => {
        ...
        const bookings = pickNumberFromKeys(row, [
          "bookings",
          "total_bookings",
          "value",
          "count",
  ...
  if (typeof raw === "object") {
    return Object.entries(raw as Record<string, unknown>)
      .map(([service, value]) => ({
        service,
        bookings: normalizeNumber(value) ?? 0,
      }))
      .sort((a, b) => b.bookings - a.bookings)
      .slice(0, 5);
  }
...
export function DashboardPage() {
  ...
  const [metricByPillar, setMetricByPillar] = useState<
    Record<string, MetricCardApiItem | null>
  >({});
  ...
  useEffect(() => {
    ...
    async function loadHuduma() {
      ...
      setHudumaError(null);
      try {
        const res = await fetch(resolveApiPath("/api/huduma"), {
          cache: "no-store",
        });
  ...
  const hudumaTotals = useMemo(() => {
    ...
    const bookings = pickNumberFromKeys(source, [
      "bookings",
      "total_bookings",
      "totalBookings",
      "total",
    ]);
    const served = pickNumberFromKeys(source, [
      "served",
      "total_served",
      "served_count",
      "servedCount",
    ]);
    const cancelled = pickNumberFromKeys(source, [
      "cancelled",
      "canceled",
      "total_cancelled",
      "cancelled_count",
    ]);
    const centers = pickNumberFromKeys(source, [
      "centers",
      "huduma_centers",
      "coverage_centers",
    ]);
    const counties = pickNumberFromKeys(source, [
      "counties",
      "coverage_counties",
    ]);
    const servedRate =
      bookings > 0 ? Math.min(100, Math.max(0, (served / bookings) * 100)) : 0;
    return { bookings, served, cancelled, centers, counties, servedRate };
  }, [hudumaData]);

  const topServices = useMemo(
    () => extractTopServices(hudumaData),
    [hudumaData],
  );
  const metrics = useMemo(
    () =>
      pillarCards.map((pillar) => {
        ...
        const resolvedUnit =
          pillar.id === "universal-health-care"
            ? "Citizens"
            : (metric?.unit ?? null);
        const progress = Math.max(
          0,
          Math.min(100, normalizeNumber(metric?.percent_value) ?? 0),
        );
        const direction = (metric?.direction ?? "").toLowerCase();
        const trend: "up" | "down" =
          direction.includes("up") || direction.includes("positive")
            ? "up"
            : "down";
        return {
          id: pillar.id,
  ...
  return (
    <div>
      ...
      >
        ...
        <div className="relative h-full flex items-center px-4 sm:px-6 lg:px-[5%] py-[25px] lg:py-16">
          <div className="max-w-4xl">
            ...
            >
              <img
                src="https://res.cloudinary.com/dirib3jmw/image/upload/q_auto/f_auto/v1775550857/5b3ad8699cb5b280b3f5817e2ec5026f64df5890_t2psf7.png"
            ...
            >
              Strategic pillars dashboard for FY 2025/26 Q3 with live KPI and
              Huduma service data.
            </motion.p>
            ...
            >
              ...
              <div className="bg-white/10 backdrop-blur-sm rounded-lg p-2 sm:p-3 md:p-4 border border-white/20">
                ...
                <div className="text-[10px] sm:text-xs md:text-sm lg:text-base font-semibold text-white">
                  Today
                </div>
              </div>
              <div className="bg-white/10 backdrop-blur-sm rounded-lg p-2 sm:p-3 md:p-4 border border-white/20">
      ...
      <div
        ...
        className="p-4 sm:p-6 px-4 sm:px-[5%] pb-[7%] space-y-4 sm:space-y-6"
      >
          ...
          <div className="flex flex-col sm:flex-row items-start justify-between gap-3">
            <div className="flex-1">
              <p className="text-gray-300 dark:text-gray-300 text-gray-700 leading-relaxed text-sm sm:text-base">
                Track real-time progress for the five strategic focus areas and
                Huduma Center service operations.
              </p>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {metrics.map((metric, index) => (
            <div key={metric.id} className="relative">
              <MetricCardEnhanced {...metric} delay={index * 0.08} />
              <Link
            ...
            <DialogTrigger asChild>
              ...
              >
                <div className="p-6 text-left">
                  ...
                  {hudumaLoading ? (
                    ...
                    <>
                      <div className="text-3xl font-bold mb-2 text-black dark:text-white">
                        {Intl.NumberFormat("en-KE").format(
                          hudumaTotals.bookings,
                        )}
                      </div>
                  ...
                  {hudumaError ? (
                    <p className="mt-3 text-xs text-rose-300">{hudumaError}</p>
                  ) : null}
                </div>
              </motion.button>
            </DialogTrigger>
            <DialogContent className="max-h-[90vh] max-w-5xl overflow-y-auto border-white/20 bg-[#0b1127] text-white">
              <DialogHeader>
                <DialogTitle className="text-xl">
                  Huduma Center Operational Dashboard
                </DialogTitle>
              </DialogHeader>
              ...
              <div className="space-y-5">
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                  ...
                  <div className="rounded-xl border border-white/15 bg-white/5 p-4">
                    ...
                    <p className="mt-1 text-2xl font-bold">
                      {Intl.NumberFormat("en-KE").format(hudumaTotals.served)}
                    </p>
                  </div>
                  <div className="rounded-xl border border-white/15 bg-white/5 p-4">
                ...
                <div className="rounded-xl border border-white/15 bg-white/5 p-4">
                  <div className="mb-2 flex items-center justify-between text-sm">
                    ...
                    <span className="font-semibold">
                      {hudumaTotals.servedRate.toFixed(1)}%
                    </span>
                  </div>
                  <div className="h-3 rounded-full bg-white/10">
                    <div
                      className="h-3 rounded-full bg-cyan-300 transition-all"
                      style={{ width: `${hudumaTotals.servedRate}%` }}
                    />
                  </div>
                </div>
                  ...
                  <div className="rounded-xl border border-white/15 bg-white/5 p-4">
                    ...
                    {topServices.length > 0 ? (
                        ...
                        {topServices.map((item) => (
                          ...
                          >
                            ...
                            <span className="font-medium text-cyan-100">
                              {Intl.NumberFormat("en-KE").format(item.bookings)}
                            </span>
                          </li>
                        ))}
                      </ul>
                      ...
                      <p className="text-sm text-gray-400">
                        No service ranking data available.
                      </p>
                    )}
                  </div>
                  ...
                  <div className="rounded-xl border border-white/15 bg-white/5 p-4">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex items-center justify-between">
                        ...
                        <span>
                          {hudumaTotals.centers > 0
                            ? hudumaTotals.centers
                            : "N/A"}
                        </span>
                      </div>
                      <div className="flex items-center justify-between">
                        ...
                        <span>
                          {hudumaTotals.counties > 0
                            ? hudumaTotals.counties
                            : "N/A"}
                        </span>
                      </div>
                    </div>
        ...
        <footer className="mt-7 text-center text-xs text-gray-300">
          Kenya Government Dashboard | Integrated strategic pillar and Huduma
